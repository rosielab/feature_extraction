...
grouped_data = data.groupby(['distance_group', category_column])[features].mean().reset_index()
...
normalized_grouped_data = pd.concat(
    [grouped_data[['distance_group', category_column]], normalized_features], axis=1
)

print('Task 1: ',normalized_grouped_data)
...
for group_name, emotions in groupings.items():
    group_data = normalized_grouped_data[normalized_grouped_data[category_column].isin(emotions)]
...
unique_emotions = normalized_grouped_data[category_column].unique()

for emotion in unique_emotions:
    # Filter data for the specific emotion
    emotion_data = normalized_grouped_data[normalized_grouped_data[category_column] == emotion]
...
def create_radar_plot_emotion_shout_levels(data, emotion, categories, title, filename):
    ...
    plt.xticks(angles[:-1], axis_labels, color='grey', size=12)
    ax.yaxis.grid(True)
    ax.set_ylim(-2.5, 2.5)  # Adjust based on the data
    plt.title(f"{title} - {emotion}", size=16, y=1.1)
    plt.legend(loc='upper right', bbox_to_anchor=(1.3, 1.1), title="Shout Levels")
...
# Group by distance group and shout level, calculate mean feature values
grouped_data = data.groupby(['distance_group', 'shout_level'])[features].mean().reset_index()

# Normalize the grouped data (normalize features across the entire grouped dataset)
normalized_features = pd.DataFrame(
    scaler.fit_transform(grouped_data[features]),
    columns=features
)
...
# Add back the distance_group and shout_level columns to the normalized data
normalized_grouped_data = pd.concat(
    [grouped_data[['distance_group', 'shout_level']], normalized_features], axis=1
)

print('Normalized Grouped Data:', normalized_grouped_data)

# Separate shout and no-shout data
shout_data = normalized_grouped_data[normalized_grouped_data['shout_level'] == 'shout ']
no_shout_data = normalized_grouped_data[normalized_grouped_data['shout_level'] == 'no-shout']

# Print for verification
print("Normalized Shout Data:")
print(shout_data)

print("\nNormalized No-Shout Data:")
print(no_shout_data)
...
def create_radar_plot_shout_no_shout(data, categories, title, filename):
    ...
    base_cmap = plt.get_cmap('tab10')  # Base colormap
    colors = base_cmap(np.linspace(0, 1, num_groups))  # Interpolate colors for the number of groups

    fig, ax = plt.subplots(figsize=(10, 8), subplot_kw=dict(polar=True))

    for i, row in enumerate(sorted_data.iterrows()):
        row_data = row[1]  # Access the row data
        values = row_data.drop(['distance_group', 'shout_level']).values.flatten().tolist()
        values += values[:1]  # Close the circle
        ax.fill(angles, values, alpha=0.25, color=colors[i], label=row_data['distance_group'])
        ax.plot(angles, values, linewidth=2, color=colors[i])
